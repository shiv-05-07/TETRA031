...
export async function generateCurriculumService(params: CurriculumParams) {
  const { subject, level, focus, format } = params;
    ...
    return {
      ...
      data: {
        ...
        bloomMapping: {
          ...
          evaluating: "10% - Model benchmarking, ethics & trade-off analysis",
...
export async function auditSyllabusService(syllabusText?: string) {
  const client = getGeminiClient();
